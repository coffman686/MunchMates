...
interface ValidDeduction {
  deduction: DeductionInput;
  item: PantryItem;
}

// Decrement quantities from several pantry items
export async function POST(req: NextRequest) {
  try {
    ...
    const results = await prisma.$transaction(async (tx) => {
      ...
      for (const { deduction, item } of validDeducations) {
