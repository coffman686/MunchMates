  ...
  const loadCollection = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const myMember = data.collection.members.find((m: CollectionMember) => m.userId === myId);
    ...
    const loadSaved = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setSavedRecipeIds(
            new Set((data.recipes || []).map((r: { recipeId: number }) => r.recipeId)),
          );
        }
      } catch {
        /* silently fail */
                    ...
                    <div className="rounded-2xl border bg-card p-5">
                      ...
                      {collection.recipes.length === 0 ? (
                        <div
                          ...
                          style={{
                            background:
                              "linear-gradient(135deg, hsl(14 80% 52% / 0.06) 0%, hsl(30 90% 55% / 0.04) 50%, hsl(350 70% 60% / 0.03) 100%)",
                          }}
                        >
                          ...
                          {collection.recipes.map((recipe) => (
                            ...
                            >
                              ...
                              <div className="flex items-center gap-1.5 ml-3">
                                ...
                                >
                                  <Heart
                                    className={`h-3.5 w-3.5 ${savedRecipeIds.has(recipe.recipeId) ? "fill-red-500 text-red-500" : ""}`}
                                  />
                                </button>
