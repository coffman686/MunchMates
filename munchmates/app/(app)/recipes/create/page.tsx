...
import { ArrowLeft, Clock, Minus, Plus, Upload, Users, X } from "lucide-react";
...
export default function CreateRecipePage() {
      ...
      try {
        ...
        setIngredients(
          (recipe.extendedIngredients || []).map(
            (ingredient: StructuredIngredient & { original?: string }) => ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      let imageUrl = imagePreview && !imagePreview.startsWith("blob:") ? imagePreview : undefined;
      ...
      const response = await authedFetch(
        isEditMode && editRecipeId
          ? `/api/recipes/create?id=${editRecipeId}`
          : "/api/recipes/create",
        {
      ...
      if (!response.ok) {
        const errData = await response.json().catch(() => null);
        throw new Error(
          errData?.error?.message || `Failed to ${isEditMode ? "update" : "create"} recipe`,
        );
      }
      ...
      alert(
        getErrorMessage(
          error,
          `Failed to ${isEditMode ? "update" : "create"} recipe. Please try again.`,
        ),
      );
    } finally {
  ...
  const cancelHref =
    isEditMode && editRecipeId ? `/recipes/${editRecipeId}` : "/recipes/my-recipes";
            ...
            <div className="flex items-center gap-4 mb-5 text-sm">
              <div className="flex items-center gap-1.5">
                ...
                <Input
                  ...
                  onChange={(e) => setReadyInMinutes(parseInt(e.target.value, 10) || 30)}
        ...
        <div className="flex flex-col-reverse md:flex-row md:gap-8 gap-6">
          ...
          <div className="md:w-[60%]">
            <div className="rounded-2xl border bg-muted/40 p-6 sm:p-8">
              ...
              {errors.steps && (
                <p className="text-sm text-destructive -mt-3 mb-4">{errors.steps}</p>
              )}
          ...
          <div className="md:w-[40%]">
            <div className="md:sticky md:top-6 rounded-2xl border bg-muted/40 p-6 sm:p-8">
              ...
              {errors.ingredients && (
                <p className="text-sm text-destructive -mt-2 mb-3">{errors.ingredients}</p>
              )}
              ...
              <div className="space-y-2">
                <div className="flex gap-2">
                  ...
                  />
                  <Select
                    value={newIngredientUnit || "__none"}
                    onValueChange={(v) => setNewIngredientUnit(v === "__none" ? "" : v)}
                  >
