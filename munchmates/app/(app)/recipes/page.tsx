...
import { FolderHeart, Heart, Plus, Search } from "lucide-react";
...
import type { RecipeSearchResult as Recipe, SavedRecipe } from "@/lib/types/recipe";
...
const Recipes = () => {
  ...
  const fetchRecipes = useCallback(async () => {
    ...
    try {
      const response = await authedFetch(
        `/api/spoonacular/recipes/searchByIngredient?${params.toString()}`,
      );
  ...
  }, [diet, intolerances, nameQuery, searchTerm, selectedCuisine, selectedDishType]);
  ...
  useEffect(() => {
    cachedIngredientList = ingredientList;
  }, [ingredientList]);
  useEffect(() => {
    cachedSearchTerm = searchTerm;
  }, [searchTerm]);
  useEffect(() => {
    cachedNameQuery = nameQuery;
  }, [nameQuery]);
  useEffect(() => {
    cachedDishType = selectedDishType;
  }, [selectedDishType]);
  useEffect(() => {
    cachedCuisine = selectedCuisine;
  }, [selectedCuisine]);
  useEffect(() => {
    cachedPantryOnly = pantryOnly;
  }, [pantryOnly]);
  useEffect(() => {
    cachedSavedIds = savedRecipeIds;
  }, [savedRecipeIds]);
  ...
  const handleWhatCanIMake = async () => {
    ...
    try {
      ...
      const data = await response.json();
      const pantryItems: string[] = (data.items || []).map((item: PantryItem) => item.canonName);
  ...
  const filteredRecipes = recipes.filter(
    (recipe) => !pantryOnly || (recipe.missedIngredientCount ?? 0) === 0,
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        >
          <SelectTrigger
            className={`h-8 w-auto rounded-full text-xs px-3 ${selectedDishType !== "All" ? "border-primary bg-primary/5" : "border-dashed"}`}
          >
        ...
        >
          <SelectTrigger
            className={`h-8 w-auto rounded-full text-xs px-3 ${selectedCuisine !== "All" ? "border-primary bg-primary/5" : "border-dashed"}`}
          >
            {selectedCuisine === "All" ? (
      ...
      {isLoading ? (
            ...
            <RecipeCard
              ...
              subtitle={
                recipe.usedIngredients?.length
                  ? `${recipe.usedIngredients.length} matching ingredient${recipe.usedIngredients.length !== 1 ? "s" : ""}`
                  : undefined
              }
            >
              <button
        ...
        /* No search yet */
        <div
          className="relative overflow-hidden rounded-2xl py-16 flex flex-col items-center text-center"
          style={{
            background:
              "linear-gradient(135deg, hsl(14 80% 52% / 0.10) 0%, hsl(30 90% 55% / 0.08) 50%, hsl(350 70% 60% / 0.05) 100%)",
          }}
        >
          ...
          <p className="text-sm text-muted-foreground max-w-xs mb-6">
            Add ingredients above to find recipe suggestions, or explore what you can cook with your
            pantry.
          </p>
