...
import { MessageCircle, Plus, Search, Star, Trash2, Utensils } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { useCallback, useEffect, useMemo, useState } from "react";
import CreatePostDialog, { type NewPost } from "@/components/community/CreatePostDialog";
...
function MuffinIcon({ filled = false, className = "" }: { filled?: boolean; className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={className} width="18" height="18" aria-hidden="true">
      {/* Muffin top (dome) */}
      <path
            ...
            <main className="flex-1 p-4 sm:p-6 bg-muted/20">
              <div className="w-full max-w-3xl mx-auto space-y-5">
                ...
                <div className="flex flex-col sm:flex-row gap-3 items-stretch sm:items-center">
                  ...
                  <Button onClick={() => setCreateOpen(true)} className="rounded-xl gap-1.5 h-10">
                        ...
                        <article key={post.id} className={`${card} overflow-hidden flex flex-col`}>
                          ...
                          <div className="flex items-center gap-2 px-5 pt-3 pb-4 mt-auto">
                            ...
                            >
                              ...
                              />
                              {post.likeCount} {post.likeCount === 1 ? "Munch" : "Munches"}
                            </button>
                            <button
                              type="button"
