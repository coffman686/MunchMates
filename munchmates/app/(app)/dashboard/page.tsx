...
// PantryItem with associated expiry date
interface PantryAlert extends PantryItem {
  expiryDate: string;
}
...
// format date of Monday of current week
...
export default function Dashboard() {
  ...
  const [pantryAlerts, setPantryAlerts] = useState<PantryAlert[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (isDietaryPrefsHydrated() && prefs.diets.length === 0 && prefs.intolerances.length === 0) {
        ...
        const migratedIntolerances = lsIntolerances
          ? lsIntolerances.split(",").filter(Boolean)
          : [];
    ...
    const loadPantryAlerts = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const expiring: PantryAlert[] = [];
          for (const item of items) {
            if (!item.expiryDate) continue;
            const days = getDaysUntilExpiry(item.expiryDate);
            if (days < 0 || days > 7) continue;
            expiring.push({ ...item, expiryDate: item.expiryDate });
          }
  ...
  // load nutrition data from nutrition-summary endpoint
  useEffect(() => {
    const loadNutrition = async () => {
      const res = await authedFetch(
        `/api/meal-plan/nutrition-summary?weekStart=${getCurrentWeekStartStr()}`,
      );
      ...
      const today: NutritionDaySummary =
        days.find((day) => day.date === todayStr) ?? days[0] ?? null;
            ...
            <main className="flex-1 p-6 bg-muted/20">
              <div className="w-full space-y-5">
                ...
                <div
                  ...
                  style={{
                    background:
                      "linear-gradient(135deg, hsl(14 80% 52% / 0.18) 0%, hsl(30 90% 55% / 0.12) 50%, hsl(350 70% 60% / 0.08) 100%)",
                  }}
                >
                  <div
                    ...
                    style={{
                      background:
                        "radial-gradient(circle, hsl(14 70% 55% / 0.15), transparent 65%)",
                    }}
                  />
                  <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex items-center gap-4 mb-0.5">
                        ...
                        <span className="text-[12px] font-medium text-primary/70 shrink-0 hidden sm:block">
                          {formatDate(new Date())}
                        </span>
                      </div>
                      <p className="text-[14px] text-muted-foreground truncate">{tipOfDay}</p>
                    </div>
                    <div className="flex items-center gap-2.5 shrink-0">
                      {[
                        {
                          label: "pantry alerts",
                          value: pantryAlerts.length,
                          color: "#FF453A",
                          href: "/pantry",
                        },
                        {
                          label: `of 21 meals`,
                          value: weeklyStats.mealsPlanned,
                          color: "#30D158",
                          href: "/meal-planner",
                        },
                        {
                ...
                {/* ── Today's Meals ── */}
                <div
                  className={`${card} p-1.5 grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-border/50`}
                >
                  <div className="pr-2 grid grid-cols-1 divide-y divide-border/50">
                    {mealSlots.map(({ key, label, icon: Icon, color, bg }) => {
                      ...
                      const content = (
                        <div
                          key={key}
                          className={`flex items-center gap-3.5 px-4 py-3.5 rounded-xl ${meal ? "hover:bg-black/[0.03] dark:hover:bg-white/[0.05] cursor-pointer" : ""} transition-colors`}
                        >
                          <div
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            {meal ? (
                              <p className="font-semibold text-[14px] truncate leading-snug mt-0.5">
                                {meal.title}
                              </p>
                            ) : (
                      ...
                      return meal ? (
                        <Link key={key} href={`/recipes/${meal.recipeId}`} className="block">
                          {content}
                        </Link>
                      ) : (
                  ...
                  {/* -- nutrition dials -- */}
                  <div className="p-6">
                    <h2 className="text-center text-xl font-bold mb-2">Nutrition Summary</h2>
                    <div className="flex flex-row justify-center gap-4">
                      {nutritionProgress.map((dial: NutritionProgressDial) => {
                        ...
                        const dialCircumPercent = Math.max(
                          0,
                          dialCircum - (percent / 100) * dialCircum,
                        );
                          ...
                          >
                            ...
                            <div className="text-center">
                              ...
                              <p className="text-xs text-muted-foreground">
                                {current} {target ? `/ ${target}` : ""} {dial.unit}
                              </p>
                            </div>
                          </div>
                        );
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        <RefreshCw
                          className={`h-3.5 w-3.5 ${popularLoading ? "animate-spin" : ""}`}
                        />
                        Refresh
                      </button>
                      <Link
                        href="/recipes"
                        className="text-[13px] font-semibold text-primary hover:text-primary/70 transition-colors flex items-center gap-0.5"
                      >
                        ...
                        >
                          <button
                            ...
                            onClick={(e) => {
                              e.preventDefault();
                              e.stopPropagation();
                              toggleSaveRecipe(recipe.id, recipe.title, recipe.image);
                            }}
                          ...
                          >
                            <Heart
                              className={`h-3.5 w-3.5 ${savedRecipeIds.has(recipe.id) ? "fill-red-500 text-red-500" : "text-white/90"}`}
                            />
                          </button>
                          <button
                            type="button"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className={card}>
                    <div className="flex items-center justify-between px-5 pt-4 pb-2">
                      ...
                      <Link
                        href="/pantry"
                        className="text-[12px] font-semibold text-primary hover:text-primary/70 transition-colors flex items-center gap-0.5"
                      >
                          ...
                          {pantryAlerts.map((item) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span
                                  className={`text-[12px] font-semibold shrink-0 ml-3 px-2 py-0.5 rounded-full ${
                                    days <= 1
