// lib/cache.ts
// Provides utilities for retrieving items from Redis cache
// and updating freshness and (in)validation

import crypto from "node:crypto";
import { ensureRedisConnected, isRedisReady, redis } from "./redis";
          ...
          if (gotLock) {
            void (async () => {
              try {
                ...
                console.warn(
                  `SWR background refresh failed for ${key}:`,
                  (err as Error)?.message ?? err,
                );
              } finally {
                await redis.del(lockKey).catch((err) => {
                  console.warn(
                    `Failed to release refresh lock ${lockKey}:`,
