...
export interface ApiErrorPayload {
  // Declare server error payload shape
  ok: false;
  error: {
    status: number;
    code: ApiErrorCode;
...
function statusToDefaultCode(status: number): ApiErrorCode {
  // Convert status to default code when caller omits explicit code
  switch (status) {
    case 400:
...
export function errorResponse(
  // Create standardized JSON response for known errors
  status: number,
  message: string,
  code: ApiErrorCode = statusToDefaultCode(status),
) {
  const payload: ApiErrorPayload = {
    // Build error payload object
    ok: false,
    error: {
      status,
      code,
...
function toApiError(error: unknown): ApiError | null {
  // Try to map unknown thrown values into typed ApiError
  ...
  if (error instanceof joseErrors.JOSEError) {
    // Map JWT/JWS verification failures to 401
    return new ApiError(401, "UNAUTHORIZED", "Unauthorized");
  }

  if (error instanceof Prisma.PrismaClientKnownRequestError) {
    // Map known Prisma request errors
    if (error.code === "P2025") {
  ...
  if (error instanceof Prisma.PrismaClientValidationError) {
    // Map Prisma validation failures
...
function formatLogError(error: unknown) {
  // Normalize unknown errors into structured object.
...
export function handleRouteError(error: unknown, context: string) {
  // Main route-level adapter for thrown errors
