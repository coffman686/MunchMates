...
import { AlertTriangle, Check, CheckCircle2, ChefHat, Loader2, Minus, XCircle } from "lucide-react";
...
export default function CookConfirmModal({
  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      ...
      try {
        ...
        const scaledIngredients = extendedIngredients.map((ing) => ({
          name: ing.name,
  ...
  const handleDeduct = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        const removed = data.results.filter(
          (r: { action: string }) => r.action === "removed",
        ).length;
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="rounded-2xl max-w-lg max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Cooking {cookServings} serving{cookServings !== 1 ? "s" : ""}. Review ingredients to
            deduct from your pantry.
          </DialogDescription>
        </DialogHeader>
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-colors ${statusColor} ${
                        row.enabled && row.match.pantryItem ? "opacity-100" : "opacity-50"
                      }`}
                    >
                      {/* Checkbox */}
                      <div
                        ...
                        <span className="text-xs text-muted-foreground">
                          ...
                          {row.match.pantryItem && row.match.pantryItem.amount !== null && (
                            <span className="ml-1 text-muted-foreground/60">
                              (have: {formatAmount(row.match.pantryItem.amount)}{" "}
                              {row.match.pantryItem.unit})
                            </span>
                          )}
                        </span>
                      </div>
